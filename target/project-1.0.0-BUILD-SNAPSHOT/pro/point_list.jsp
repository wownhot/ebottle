...
<nav class="navbar navbar-inverse">
	...
	<div class="container">
		...
		%>
			회원님이 적립하신 포인트는 <%=request.getAttribute("STACKPOINT")%> Point입니다.</h3></p>
		<% 	
		}else if(request.getAttribute("STACKPOINT")==null && request.getAttribute("SPENDPOINT")==null){
		%>
		현재 회원님이 보유하신 포인트는 <%=request.getAttribute("POINT")%> Point입니다.</h3></p>
		<% 
		}else if(request.getAttribute("STACKPOINT")==null && request.getAttribute("POINT")==null){
		%>
		회원님이 사용하신 포인트는 <%=request.getAttribute("SPENDPOINT")%> Point입니다.</h3></p>
		<%
		}
		}else{
			...
			if(request.getAttribute("POINT")==null && request.getAttribute("SPENDPOINT")==null){
				%>
				<%=mm %>월 회원님이 적립하신 포인트는 <%=request.getAttribute("STACKPOINT")%> Point입니다.</h3></p>
				<% 	
				}else if(request.getAttribute("STACKPOINT")==null && request.getAttribute("SPENDPOINT")==null){
				%>
				<%=mm %>월
				...
				%>
				<%=mm %>월 회원님이 사용하신 포인트는 <%=request.getAttribute("SPENDPOINT")%> Point입니다.</h3></p>
				<%
				}
		}
		%>
      ...
      <p ><h3 style="text-align:center">포인트 내역</h3></p>
      <form method="post" action="/monthlist?currentPage=1">
     <p>
     	<%if(request.getAttribute("MON")==null){ %>
