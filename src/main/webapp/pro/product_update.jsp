 ...
 function goPage(prm) {
	 if(prm=='update'){
		 ...
		 document.f.submit();
  			...
  			document.ff.action = "/productmodify";
  			document.ff.submit();
  	 } else if(prm == "delete") {
  			document.ff.action = "/productdelete";
  			document.ff.submit();
  	}else if(prm=='grade'){
...
<nav class="navbar navbar-inverse">
 ...
 %></li>
	...
	<% ProductVO po = new ProductVO();
	   po = (ProductVO)session.getAttribute("PRODUCT_UPDATE"); 
	   
	   %>
	   
	<form class="form-signin" action="/UserUpdateServlet" method="get"
				name="ff">
						...
						<div class="col-md-4 col-md-offset-4">
							<div class="panel panel-default">
								<div class="panel-body">
									<h3 class="text-center">상품정보수정</h3>
									<form class="form form-signup" role="form">
										<div class="form-group">
											<div class="input-group">
												<span class="input-group-addon"><span
													class="glyphicon glyphicon-pencil"></span> </span> <input
													type="text" placeholder="상품명을 입력하세요" name="pname" class="form-control"
													value="<%=po.getPname()%>" />
											</div>
										</div>
										...
										<div class="form-group">
											<div class="input-group">
												<span class="input-group-addon"><span
													...
													type="text" placeholder="상품가격을 입력하세요" name="price" class="form-control"
													value="<%=po.getPrice()%>" />
											</div>
										</div>
										<div class="form-group">
											<div class="input-group">
												<span class="input-group-addon"><span
													class="glyphicon glyphicon-list-alt"></span></span> <input
													type="text" placeholder="상품재고를 입력하세요" name="stock" class="form-control" 
													value="<%=po.getStock()%>" />
											</div>
										</div>
										<div class="form-group">
													...
													<label for="comment">상품설명:</label>
  													<textarea class="form-control" rows="5" name="desc" id="comment" value="<%=po.getDesc()%>"><%=po.getDesc()%></textarea> 
													<%-- <input
													type="text" name="desc" placeholder="상품설명을 입력하세요" class="form-control" 
													value="<%=po.getDesc()%>" /> --%>
									...
									<button type="button" class="btn btn-sm btn-primary btn-block" onClick="goPage('modify')">상품 수정</button>
        							<button type="button" class="btn btn-sm btn-danger btn-block" onClick="goPage('delete')">상품 삭제</button>
								</div>
	</div>
	</div>
	</div>
